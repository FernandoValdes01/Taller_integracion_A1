<?php
    ...
    $query = "SELECT foto, Nombre_Trabajador, profesion, Rut_Trabajador, Correo_Trabajador FROM trabajador WHERE Correo_Trabajador = '$correoTrabajador'";
    ...
    <?php
        ...
        if (mysqli_num_rows($result) > 0) {
            while ($row = mysqli_fetch_assoc($result)) {
                ...
                echo '<div id="descripcion-' . $row['ID_solicitud'] . '" style="display: none;">';
                echo "Descripción del servicio: " . $row['descripcion'] . "<br>";
                echo '</div>';

                echo '<button onclick="mostrarDescripcion(' . $row['ID_solicitud'] . ')">Mostrar Descripción</button>';
